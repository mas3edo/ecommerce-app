...
import { Mail, Lock, User, ArrowRight, ShieldCheck, AlertCircle, CheckCircle, Eye, EyeOff } from "lucide-react";
...
export default function AuthComponent() {
  ...
  // ✅ If user already has a valid session/token, redirect to Home
  useEffect(() => {
  ...
  const toggleView = (newView) => {
    ...
    // Keep email if they typed it, reset others
    setOtp("");
    setPassword("");
    setConfirmPassword("");
  ...
  const handleRegister = async (e) => {
    ...
    try {
      // 2. إرسال الطلب لـ Supabase (إيميل وباسورد فقط)
      const { data, error } = await supabase.auth.signUp({
        email: email,
        password: password,
      });

      // 3. مسك الأخطاء الواضحة من السيرفر (زي الباسورد القصير)
      if (error) {
      ...
      // 4. الفحص السري: هل الإيميل متسجل قبل كده؟
      ...
      // 5. لو كله تمام، طلع رسالة النجاح
      setSuccessMessage('Account created successfully! 🎉 Please log in.');

      // لو عندك دالة بتبدل الشاشة للوجن، شيل علامتين الـ // من السطر اللي جاي
      // setView('login'); 

    } catch (err) {
      // 6. لو حصلت مشكلة في النت أو السيرفر
